...
class data_export_helper {
    ...
    public static function export_discussion_data($userid, array $mappings) {
        ...
        $sql = "SELECT
                    ...
                    dsub.preference
                  FROM {overflow} mof
            INNER JOIN {overflow_discussions} d ON d.overflow = mof.id
            LEFT JOIN {overflow_discuss_subs} dsub ON dsub.discussion = d.id
        ...
        foreach ($discussions as $discussion) {
            $forumswithdata[$discussion->overflow] = true;
            $context = \context::instance_by_id($mappings[$discussion->overflow]);
    ...
    public static function export_all_posts($userid, array $mappings) {
        ...
        $sql = "SELECT
                  ...
                  FROM {overflow} mof
            INNER JOIN {overflow_discussions} d ON d.overflow = mof.id
            INNER JOIN {overflow_posts} p ON p.discussion = d.id
            LEFT JOIN {overflow_read} fr ON fr.postid = p.id
            LEFT JOIN {overflow_ratings} rat ON  rat.postid = p.id
     ...
     * Store all information about all posts that we have detected this user to have access to.
     * @param int       $userid     The userid of the user whose data is to be exported.
     * @param \context  $context    instance of the overflow context.
    ...
    protected static function export_all_posts_in_discussion($userid, \context $context, \stdClass $discussion) {
        ...
        $sql = "SELECT
              ...
              INNER JOIN {overflow_posts} p ON p.discussion = d.id
              LEFT JOIN {overflow_read} fr ON fr.postid = p.id AND fr.userid = :readuserid
              LEFT JOIN {overflow_ratings} rat ON rat.id = (
                SELECT MAX(id) FROM {overflow_ratings} ra
        ...
        $discussionarea[] = get_string('posts', 'local_overflow');
    ...
    protected static function export_post_data($userid, \context $context, $postarea, $post) {
        ...
        $postdata->message = writer::with_context($context)->rewrite_pluginfile_urls(
            $postarea, 'local_overflow', 'attachment', $post->id, $post->message);
        ...
        writer::with_context($context)->export_data($postarea, $postdata)->export_area_files(
            $postarea, 'local_overflow', 'attachment', $post->id);
    ...
    protected static function export_rating_data($postid, $onlyuser, $userid) {
        ...
        $ratingpost = $rating->overflow_get_rating($postid);
    ...
    public static function export_subscription_data(\stdClass $forum) {
        if (null !== $forum->subscribed) {
            ...
            writer::with_context(\context_system::instance())->export_metadata(
                [], 'subscriptionpreference', 1, get_string('privacy:subscribedtoforum', 'local_overflow'));
     ...
     *
     * @param \context_system $context      instance of the overflow context.
     * @param \stdClass       $discussion   The discussion whose data is being exported.
     *
     * @return bool         Whether any data was stored.
     */
    protected static function export_discussion_subscription_data(\context_system $context, \stdClass $discussion) {
        ...
        if (null !== $discussion->preference) {
            ...
            switch ($discussion->preference) {
                case \local_overflow\subscriptions::OVERFLOW_DISCUSSION_UNSUBSCRIBED:
                    $a->preference = get_string('unsubscribed', 'local_overflow');
                    break;
                default:
                    $a->preference = get_string('subscribed', 'local_overflow');
                    break;
            }
            writer::with_context($context)->export_metadata(
                    ...
                    get_string('privacy:discussionsubscriptionpreference', 'local_overflow', $a)
                );
    ...
    /**
     * Store overflow read-tracking data about a particular overflow.
     * This is whether a overflow has read-tracking enabled or not.
     *
     * @param \stdClass $forum  The overflow whose data is being exported.
     *
     * @return bool      Whether any data was stored.
     */
    public static function export_tracking_data(\stdClass $forum) {
        if (null !== $forum->tracked) {
            ...
            writer::with_context(\context_system::instance())->export_metadata(
                [], 'trackreadpreference', 0, get_string('privacy:readtrackingdisabled', 'local_overflow'));
    ...
    public static function export_grade_data(\stdClass $forum) {
        if ($forum->grade) {
            writer::with_context(\context_system::instance())->export_metadata(
                    [], 'grade', $forum->grade, get_string('privacy:grade', 'local_overflow'));
     ...
     *
     * @param \context_system $context  The instance of the forum context.
     * @param array           $postarea The subcontext for this post.
     * @param \stdClass       $post     The post whose data is being exported.
     *
     * @return bool     Whether any data was stored.
     */
    protected static function export_read_data(\context_system $context, array $postarea, \stdClass $post) {
        if (null !== $post->firstread) {
            ...
            writer::with_context($context)->export_metadata(
                    ...
                    get_string('privacy:postwasread', 'local_overflow', $a)
                );
    ...
    protected static function get_discussion_area(\stdClass $discussion) {
        ...
        $pathparts[] = get_string('discussions', 'local_overflow');
    ...
    /**
     * Get the parent subcontext for the supplied overflow, discussion, and post combination.
     *
    ...
    protected static function get_post_area_for_parent(\stdClass $post) {
        global $DB;
        $subcontext = [];
        if ($parent = $DB->get_record('overflow_posts', ['id' => $post->parent], 'id, created')) {
    ...
    /**
     * Get the subcontext for the supplied overflow, discussion, and post combination.
     *
    ...
    public static function get_subcontext($discussion = null, $post = null) {
        ...
        if (null !== $discussion) {
            ...
            if (null !== $post) {
                $subcontext[] = get_string('posts', 'local_overflow');
